...
void PropheseeWrapperPublisher::publishCDEvents() {
    ...
    try {
        Metavision::CallbackId cd_callback =            //~  called when this add_callback works.
            camera_.cd().add_callback([this](const Metavision::EventCD *ev_begin, const Metavision::EventCD *ev_end) {
                ...
                // if (pub_cd_events_.getNumSubscribers() <= 0)         //~ commented to show imformation
                //     return;
                ...
                if ((event_buffer_current_time_ - event_buffer_start_time_) >= event_delta_t_) {
                    ...
                    // ROS_INFO_STREAM("CD data available, buffer size: " << static_cast<int>(event_buffer_msg.events.size()));
                }
